---
title: "Application of poisson mash and mash to simulated single cell data (setting 3)"
author: "Yusha Liu"
header-includes:
date: "2020-11-1"
...
\noindent Scenario 2. Treatment effect is added to $500$ genes randomly chosen from the genes with relatively high expression (> 70 percentile). The effect size $w_j$ is drawn from the set $\{\log_2(1.5), \log_2(2), \log_2(2.5), \log_2(3)\}$ with probability $\{0.2, 0.5, 0.2, 0.1\}$, and further added an $N(0, 0.1^2)$ noise on top of that.

\noindent Scenario 3. Treatment effect is added to $500$ genes randomly chosen from the genes with relatively lower expression levels (35 ~ 70 percentile). The effect size $w_j$ is drawn from the set $\{\log_2(2), \log_2(3), \log_2(4), \log_2(5)\}$ with probability $\{0.3, 0.4, 0.2, 0.1\}$, and further added an $N(0, 0.1^2)$ noise on top of that.


## Scenario 1 (no treatment effect)
```{r warning=FALSE, message=FALSE}
res <- readRDS("output/B_cells/simulations3/null_random_effects_summary.Rds")
...
```{r warning=FALSE, message=FALSE}
res <- readRDS("output/B_cells/simulations3/sim_signal_upr_summary.Rds")
alpha <- res$alpha
...
```{r warning=FALSE, message=FALSE}
res <- readRDS("output/B_cells/simulations3/sim_signal_lwr_summary.Rds")
alpha <- res$alpha
